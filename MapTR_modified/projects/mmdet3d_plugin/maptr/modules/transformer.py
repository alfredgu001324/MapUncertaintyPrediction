    ...
    def get_bev_features(
        ...
        if self.use_attn_bev:
            bev_embed = self.attn_bev_encode(
                ...
                bev_queries,                 # [20000, 256]
                bev_h,                       # 200
                bev_w,                       # 100
                grid_length=grid_length,     # (0.3 0.3)
                bev_pos=bev_pos,             # [1,256,200,100]
                prev_bev=prev_bev,
                **kwargs)
        else:
...
def bev_vis(bev_embed):

    # Reducing the last dimension to 3 using PCA
    pca = PCA(n_components=256)
    reduced_data = np.zeros((20000, 256))

    reduced_data = pca.fit_transform(bev_embed[0])
