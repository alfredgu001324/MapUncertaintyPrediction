...
model = dict(
    ...
    pts_bbox_head=dict(
        ...
        loss_pts=dict(type='PtsNLLLoss', 
                      loss_weight=0.03), #5   #gus 0.5
        loss_dir=dict(type='PtsDirCosLoss', loss_weight=0.01)), #0.005   #gus 0.5
    ...
    train_cfg=dict(pts=dict(
        ...
        assigner=dict(
            type='MapTRAssigner',
            cls_cost=dict(type='FocalLossCost', weight=2), #2
...
data_root = '/home/data/argoverse2_mini/sensor/'
...
optimizer = dict(
    type='AdamW',
    lr=1.5e-4, #6  #gus 0.05
    paramwise_cfg=dict(
        custom_keys={
            'img_backbone': dict(lr_mult=0.1),
        }),
    weight_decay=0.01) #0.01   # gus 0.00125

optimizer_config = dict(grad_clip=dict(max_norm=3, norm_type=2)) #max_norm=35
# learning policy
lr_config = dict(
